...
export const Guests: React.FC = () => {
  ...
  const [formInvStatus, setFormInvStatus] = useState('sent');
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    const payload: Guest = {
      ...
      invitationStatus: formInvStatus || undefined,
    ...
    try {
      ...
      console.error(err);
      toast.error('Failed to save guest record.');
    }
  };
  ...
  const handleOpenEditModal = (guest: Guest) => {
    ...
    setFormInvStatus(guest.invitationStatus || 'sent');
    setIsAddEditModalOpen(true);
  };
  ...
  const processedGuests = useMemo(() => {
    ...
    if (searchQuery.trim()) {
      ...
      result = result.filter(g => 
        ...
        (g.invitationStatus && g.invitationStatus.toLowerCase().includes(q))
      );
    }
  ...
  const handleExportCSV = () => {
    ...
    const headers = ['Name', 'Phone', 'Side', 'RSVP Status', 'VIP', 'Family Group', 'Notes', 'Village', 'Inv Status', 'Created At'];
    ...
    link.setAttribute("download", `project_ra_guests_${currentWorkspaceId}_${currentWeddingId}.csv`);
  ...
  const handleExportExcel = () => {
    ...
    const link = document.createElement("a");
    link.href = url;
    link.download = `project_ra_guests_${currentWorkspaceId}_${currentWeddingId}.xls`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  ...
  return (
    <div className="space-y-8 font-poppins relative">
      ...
      <div className="hidden print-header">
        ...
        <p className="text-xs text-black/60 uppercase tracking-widest mt-1">Workspace Guest Manifest Summary</p>
        <p className="text-[10px] text-black/40 mt-0.5">Workspace: {currentWorkspaceId} • Wedding: {currentWeddingId}</p>
        ...
        <div className="space-y-4">
          ...
          <div className="relative w-full">
            <input
              ...
              placeholder="Search by name, phone, family group, village or status..."
          ...
          <table className="w-full print-table text-left border-collapse text-xs font-poppins">
            ...
            <tbody className="divide-y divide-[#D4AF37]/5 text-[#F5F5F5]/85">
              {paginatedGuests.length === 0 ? (
                <tr>
                  <td colSpan={9} className="p-8 text-center text-xs text-[#F5F5F5]/30">
                ...
                paginatedGuests.map((guest) => (
                  <tr key={guest.id} className="hover:bg-[#141414]/30 transition duration-150">
                    ...
                    <td className="p-4 text-[#F5F5F5]/70">{guest.village || '—'}</td>
                    <td className="p-4">
                      <span className="text-[10px] bg-zinc-900 border border-zinc-800 text-zinc-400 px-2 py-0.5 rounded-full capitalize">
                      ...
                      <table className="w-full text-left text-xs font-poppins print-table">
                        <thead>
                          <tr className="text-[#D4AF37]/60 text-[10px] uppercase font-semibold">
                            ...
                            <th className="pb-2 pr-4">Invitation</th>
                        ...
                        <tbody className="divide-y divide-[#D4AF37]/5 text-[#F5F5F5]/80">
                          {members.map(member => (
                            <tr key={member.id} className="hover:bg-[#141414]/20 transition-all">
                              ...
                              <td className="py-2.5 text-[#F5F5F5]/60">{member.village || '—'}</td>
                              <td className="py-2.5 capitalize">{member.invitationStatus || 'sent'}</td>
      ...
      >
        <form onSubmit={handleSave} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            />
            <div className="space-y-1.5">
              <label className="text-[11px] uppercase tracking-wider text-[#F5F5F5]/60 font-semibold block">Invitation Status</label>
              <select
                value={formInvStatus}
                onChange={(e) => setFormInvStatus(e.target.value)}
                className="w-full text-sm bg-[#090909] border border-[#D4AF37]/15 focus:border-[#D4AF37]/50 text-[#F5F5F5] rounded-xl px-3 py-3 focus:outline-none transition cursor-pointer font-poppins"
              >
