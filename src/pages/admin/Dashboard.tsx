...
import { getGuests, getActivityLogs } from '../../services/guestService';
...
export const Dashboard: React.FC = () => {
  ...
  const handleExportList = () => {
    // Forward export action
    toast.success('CSV guest list generated.', { id: 'export-toast' });
    ...
    link.setAttribute("download", `project_ra_guests_export.csv`);
  ...
  const handleRefresh = () => {
    ...
    toast.loading('Synchronizing dashboard telemetry...', { id: 'refresh-toast' });
    setTimeout(() => {
      loadDatabaseState();
      setIsRefreshing(false);
      toast.success('Dashboard telemetry updated.', { id: 'refresh-toast' });
    }, 1000);
  };
  ...
  // Helper for timeline badges/colors
  const getActionColorClass = (action: string): string => {
    const act = action.toLowerCase();
    if (act.includes('add')) return 'bg-[#D4AF37]';
      ...
      <motion.div variants={itemVariants} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-[#D4AF37]/10 pb-6">
        <div className="flex flex-col gap-1">
          ...
          <p className="text-xs text-[#F5F5F5]/50">
            Welcome back, <span className="text-[#D4AF37] font-semibold">{user?.displayName || 'Administrator'}</span>. Here is the operational summary of your invitation experience.
          </p>
        </div>
                    ...
                    <div key={log.id} className="relative group">
                      ...
                      <div className="flex items-start justify-between gap-4 p-3.5 rounded-xl hover:bg-[#141414]/50 border border-transparent hover:border-[#D4AF37]/5 transition-all duration-300">
                        ...
                        <span className={`text-[9px] tracking-wider font-semibold uppercase px-2 py-0.5 rounded-full ${
                          ...
                          log.action.includes('Add') ? 'bg-[#D4AF37]/15 text-[#D4AF37] border border-[#D4AF37]/30' :
            ...
            <Card className="w-full">
              <div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-center gap-3.5 p-3.5 rounded-xl bg-[#090909] border border-[#D4AF37]/10">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-[#F5F5F5]/45 mt-0.5">
                        Sprint B (Guest Management) v1.2.0
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="mt-6 p-4 rounded-xl bg-[#0F6D5B]/5 border border-[#0F6D5B]/20 text-[11px] text-[#F5F5F5]/70 leading-relaxed font-poppins">
                  ...
                  This portal acts as Sprint B (Guest Management). In Sprint C, the live RSVP synchronization, seating, and CSV imports will be fully integrated.
                </div>
              </div>
            </Card>
          </motion.div>
